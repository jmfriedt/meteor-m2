...
// gcc -o jmf_libfec jmf_libfec_blocks_rs.c -I./libfec ./libfec/libfec.a 
...
{int i;
 for (i=0;i<223;i++) rsbuff[i]=data[i*I+pos];                   // Copy data
 for (i=0;i<32; i++) rsbuff[i+223]=data[PARITY_OFFSET+i*I+pos]; // Copy parity
}
...
int main(int argc,char *argv[]){
 ...
 do {
  res=read(fdi,symbols,framebits*2+50);
...
//  write(fdo,data,MAXBYTES);  // ecriture avant le XOR qui sera fait dans Octave
  for (i=0;i<12;i++) printf("%02hhX",data[i]);
  tmp=&data[4];
  for (i=0;i<1020; i++) tmp[i]^=pn[i%255];

  for (i=0; i<RSBLOCKS; i++) 
      {// deinterleaveRS(tmp, rsWorkBuffer, i, RSBLOCKS);
        for (j=0;j<255; j++) rsWorkBuffer[j]=tmp[j*4+i];
        for (j=0;j<255; j++) rsWorkBuffer[j]=ToDualBasis[rsWorkBuffer[j]];
        derrors[i] = decode_rs_ccsds(rsWorkBuffer, NULL, 0, 0);
        for (j=0;j<255; j++) rsWorkBuffer[j]=FromDualBasis[rsWorkBuffer[j]];
        interleaveRS(rsWorkBuffer, rsCorrectedData, i, RSBLOCKS);
        printf(":%d",derrors[i]);
      }
  printf("->",res);
  for (i=0;i<15;i++) printf("%02hhX",rsCorrectedData[i]);
  printf(" %d\n",res);
  write(fdo,data,4);                      // header
  write(fdo,rsCorrectedData,MAXBYTES-4);  // corrected frame
